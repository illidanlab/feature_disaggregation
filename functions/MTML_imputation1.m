function [Wl,Wr,G,Xlr,Xlrtst,fval] =  MTML_imputation1(y,Xl,Xltst,Xr_bar,lambda1,lambda2,lambda3,lambda4,coords,G_init,trnidx,tstidx,maxiter,toler)
...
%     R = sparse(double(R));
    ...
    fval(1) = primal_fval(y,Xl,Xlrall_old,Xr_bar_total,rand(dl,r),rand(dr,r),G_old,randn(r,1),lambda1,lambda2,lambda3,lambda4,D);
    %BCD starts here
    for i = 2:maxiter
        %fix Xlr, G, solving Wl,Wr;
        [b,Wl,Wr] = solve_WlWr(y,Xl,Xlr_oldtrn,G_old,lambda1);
        %fix Wl,Wr,Xlr, solving G;
        Wl_old = Wl;
        Wr_old = Wr;
        b_old = b;
        G = solve_G(y,Xl,Xlr_oldtrn,Wl_old,Wr_old,b_old,lambda2);
        ytmpymp = y;
        for ii = 1:r
            ytmptmp{ii} = y{ii} - b(ii);
        end
        [Xlr_total,~] = Xlrsolver_FISTA(ytmptmp,Xl,G,R,Wl_old,Wr_old,Xr_bar,Xlrall_old,lambda3,lambda4,trnidx,500,0.01);
        Xlrall_old = cell(1,r);
        ind = 1;
        for j = 1:r
        ...
        G_old = G;
        fval(i,1) = primal_fval(y,Xl,Xlrall_old,Xr_bar_total,Wl,Wr,G,b,lambda1,lambda2,lambda3,lambda4,D);
        Xlr_oldtrn = cell(1,r);
        for regionid = 1:r
...
% nested function
    %% 
    function [fval] = primal_fval(y,Xl,Xlrall,Xr_bar_total,Wl,Wr,G,b,lambda1,lambda2,lambda3,lambda4,~)
        fval = 0;
        Xlr = cell(1,r);
        for regionidx = 1:r
...
%         conttable = zeros(50*49/2,1)
        ...
        for region = 1:r
            rmse = norm(y{region}-Xl{region}*Wl(:,region) - Xlr{region}*Wr(:,region)-diag(Xl{region}*G*Xlr{region}')-b(region))^2;
            fval = fval + rmse  + lambda1 * l1_mat(vertcat(Wl,Wr)) + lambda2 * l1_mat(G) + lambda3 * contRegu ...,
                    + lambda4 * discRegu; 
        end
      
    end

    function [b,Wl,Wr] = solve_WlWr(y,Xl,Xlr_old,G_old,lambda1)
        ...
        for region = 1:r
            ...
            X_all{region} = horzcat(ones(size(Xl{region},1),1),Xl{region},Xlr_old{region});
        end
        W = Least_Lasso(X_all,yhat,lambda1);
        b = W(1,:);
        Wl = W(2:dl+1,:);
        Wr = W(dl+2:end,:);
    end
    
    function G = solve_G(y,Xl,Xlr_old,Wl_old,Wr_old,b_old,lambda2)
        %solve global regression for G
        y_all = cell(1,r);
        for region = 1:r
            y_all{region} = y{region} - Xl{region}*Wl_old(:,region) - Xlr_old{region}*Wr_old(:,region)-b_old(region);
        end
        y_all = cat(1,y{:});
