function [Wl,Wr,G,Xlr,Xlrtst,fval] =  MTML_imputation_mean(y,Xl,Xltst,Xr_bar,lambda1,lambda2,lambda3,lambda4,coords,G_init,trnidx,tstidx,maxiter,toler)
...
%     R = sparse(double(R));
    ...
    fval(1) = primal_fval(y,Xl,Xlrall_old,Xr_bar_total,rand(dl,r),rand(dr,r),G_old,lambda1,lambda2,lambda3,lambda4,D);
    %BCD starts here
    for i = 2:maxiter
        %fix Xlr, G, solving Wl,Wr;
        [Wl,Wr] = solve_WlWr(y,Xl,Xlr_oldtrn,G_old,lambda1);
        %fix Wl,Wr,Xlr, solving G;
        Wl_old = Wl;
        Wr_old = Wr;
        G = solve_G(y,Xl,Xlr_oldtrn,Wl_old,Wr_old,lambda2);
%         [Xlr_total,~] = Xlrsolver_FISTA(y,Xl,G,R,Wl_old,Wr_old,Xr_bar,Xlrall_old,lambda3,lambda4,trnidx,500,0.01);
        [Xlr_total,~] = Xlrsolver_FISTA_mean(y,Xl,G,R,Wl_old,Wr_old,Xr_bar,Xlrall_old,lambda3,lambda4,trnidx,500,0.01);
        Xlrall_old = cell(1,r);
        ind = 1;
        for j = 1:r
