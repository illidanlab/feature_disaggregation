...
rng(2);
r = 10;% # region
n = 100;% # sample size in each region
dl = 20; % # local feature
dr = 1; % # region feature for visualization
...
ind = 1;
for i = 1:r
     X_R_local{i}(:,1) = tmp(ind:ind+n-1,1);
...
for i = 1:r
    alphatrue(:,i) = alphatrue(:,i);
end
% alphatrue = normrnd(0,3,[dl,r]);
betatrue = randn(dr,r);
for i = 1:r
    betatrue(:,i) = betatrue(:,i);
end
...
rng('shuffle') 
for i = 1:r
    Y{i} = X_L{i}*alphatrue(:,i) + (X_R_local{i}*betatrue(:,i)) + X_csi{i}*gammatrue +  randn(n,1)./10;
...
tic;
% Load data
clearvars -except F1_imput dl dr F1_CLI F1_MTMLa Sign_imput Sign_CLI Sign_MTMLa Gcli X_R_local cvind coord G_Leastlasso G_estimateint G_dirty ROUND alphatrue betatrue gammatrue X_csi_syn X_L X_R Y cvind Xtrn Xtst Ytrn Ytst r d dl dr dcsi varname Wtrue Gtrue;
...
bestpara = [0,0.55,0.002,5];
[Wl,Wr,Gestimate,Xlr,Xlrtst,fval] = MTML_imputation1(Ytrn,Xtrn,Xtst,X_R,bestpara(1),bestpara(2),bestpara(3),bestpara(4),coord,randn(dl,dr),trainIdx,testIdx,50,1e-5);
...
[b,Wl,gamma] =  cv_solver(Ytrn,Xtrn,Xlrtrn,0,0,Gestimate,50,1e-7);
...
for i = 1:r
    Xtst_noise{i} = horzcat(ones(size(Xtst{i},1),1),Xtst{i},Xlrtst{i},Xcsitst{i});
end
...
minimize(norm((W - G*X_z),'fro') + 10*l1_mat(G))
cvx_end
G = G(:,1:dr);
G = (abs(G)>0.00001).*G;
[cm, accuracy, precision, recall, F1] = CalcSparse(Gtrue,G);
...
[b,Wl,gamma] =  cv_solver(Ytrn,Xtrn,Xreg_trn,1,0.05,randn(dl,dr),50,1e-3);
gammamat = repmat(gamma,[1,r]);
W = vertcat(Wl,gammamat);
W = vertcat(b,W);
Xreg_tst = cell(1,r);
...
for i = 1:r
    Xtst_noise{i} = horzcat(ones(size(Xtst{i},1),1),Xtst{i},Xreg_tst{i},Xcsitst{i});
end
[rmse,rmseall,r2,r2all,ypred,yreal]= eval_rmse2(Xtst_noise,Ytst,W);
...
result{method+1,7} = ypred;result{method+1,8} = yreal;result{method+1,9} = W;result{method+1,10} = 'best_param';result{method+1,11} = 'perform_mat';result{method+1,12} = 'Fval';result{method+1,13} = G;result{method+1,14} = cm;result{method+1,15} = cmsign;result{method+1,16} = [accuracysign];result{method+1,17} = [F1];
clear yreal ypred r2 rmse r2all rmseall best_param perform_mat W Fval;
fname = ['Syn5region3_','28_',num2str(ROUND)];
save(fname);
result
F1_imput(ROUND) = result{2,16};
F1_CLI(ROUND) = result{5,16};
F1_MTMLa(ROUND) = result{9,16};
Sign_imput(ROUND) = result{2,17};
Sign_CLI(ROUND) = result{5,17};
Sign_MTMLa(ROUND) = result{9,17};
% bar(cat(1,X_R_local{:}))
% figure
% bar(cat(1,Xlr{:}))
end 
