 function [Xlr,fval] = Xlrsolver_FISTA_mean(y,Xl,G,R,Wl_old,Wr_old,Xr,Xlrold,lambda3,lambda4,trnidx,maxiter,tol)
...
%% nested function
    ...
    function [fval] = primal_fval(y,Xl,Xlr,Wr,G,lambda3,lambda4,R)
       ...
       fval = norm(y-Xlrtrn*Wr-diag(Xl*G*Xlrtrn')).^2 + lambda3*clustloss + lambda4 * norm(M*Xlr*p-xr_mean,'fro').^2; 
    end
     ...
     function [gradXlr] = grad_Xlr(y,Xl,G,w,Xr,Xlrold,lambda3,lambda4)
            ...
            gradXlr = sqgrad + lambda3*clusgrad(Xlrold) + lambda4*meangrad(Xlrold,xr_mean);
     end
 end